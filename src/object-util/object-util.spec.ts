...
describe('isEqualObject', () => {
  it('should return true', () => {
    expect(isEqualObject({ foo: 1 }, { foo: 1 })).toBe(true);
    expect(isEqualObject({ foo: { bar: 'baz' } }, { foo: { bar: 'baz' } })).toBe(true);
    ...
    expect(isEqualObject(new RegExp('ab+c'), new RegExp('ab+c'))).toBe(true);
  });

  it('should return false', () => {
    expect(isEqualObject(new RegExp('ab+c'), new RegExp('ab+d'))).toBe(false);
    expect(isEqualObject({ foo: 1 }, { foo: 2 })).toBe(false);
    expect(isEqualObject({ one: 1, two: 2 }, { one: 1, two: 2, three: 3 })).toBe(false);
    expect(isEqualObject([1, 2], [1, 2, 3])).toBe(false);
    ...
    expect(isEqualObject({ foo: fn('a') }, { foo: fn('b') })).toBe(false);
    const symbol = Symbol('a');
    expect(isEqualObject({ [symbol]: { a: 'foo' } }, { [symbol]: { a: 'bar' } })).toBe(false);
  });
});
